...
function padWithZeros(p) {
  return utils.hexZeroPad(p,32);
}
...
class ECKeyPair {
  ...
  // @desc Encrypt a random scalar with the instance's public key
  ...
  async encrypt(number) {
    ...
    const privkey = new ethers.utils.SigningKey(ephemeralKey.privateKey); 
    // compute ecdh secret
    const secret = privkey.computeSharedSecret(this.publicKeyHex);
    
    // c = secret ^ number
    const ciphertext = number.value.xor(secret);
    
    // Return ciphertext and public key (include MAC?)
    const result = {      
      ephemeralPublicKey: ephemeralKey.publicKey,
      ciphertext: padWithZeros(ciphertext.toHexString()),
    };
    return result;
  }
  ...
  async decrypt(output) {
    // drop buffer loading replace with ecdh + xor
    
    // load local private key
    const privkey = new ethers.utils.SigningKey(this.privateKeyHex); 
    // load secret
    const secret = privkey.computeSharedSecret(output.ephemeralPublicKey)
   
    // Decrypt data
    const plaintext = ethers.BigNumber.from(output.ciphertext).xor(secret);
    // Return value as string
    return padWithZeros(plaintext.toHexString());
  }
